...
settings.onlyClusters = 0; % 1 = don't plot non-significant data, replace it with zero value; 0 = plot all data, with clusters as a contours
settings.TopoWithMask = 1; % whether to plot mask
...
settings.prestimulusOnly = 1; % 1 = prestimulus part of paradigm on plot; 0 = prestimulus and poststimulus part on plot;

if settings.prestimulusOnly == 1
    settings.prefix = 'final_p05_'; % additional prefix for naming plots
elseif settings.prestimulusOnly == 0
    settings.prefix = 'supp_p05_'; % additional prefix for naming plots
end
...
elseif settings.inverted== 1 & settings.intercept== 0
    ...
    pathBETAS = [pathTFData '\\betas_odwrotne_nozscore\']
...
elseif settings.inverted== 0 & settings.intercept== 0
    ...
    pathBETAS = [pathTFData '\\betas_nozscore\']
...
settings.electrodes_distance = 50;
chan_hood = spatial_neighbors(chanlocs(1:64), settings.electrodes_distance); % 40 mm distance between electrodes seems to be working with EEGLAB headset.
...
for n=1:length(fnames)
    ...
    [cluster.(fnames{n,1}).cluster, cluster.(fnames{n,1}).p_values, cluster.(fnames{n,1}).t_sums, cluster.(fnames{n,1}).permutation_distribution ] = permutest(betas.selected_electrodes.(fnames{n,1})(:, :, :), zero, true, settings.p_val, settings.perm, true, inf);
...
for i=1:length(fnames)
    ...
    if  settings.prestimulusOnly == 0
        ...
        result_mean_data.(fnames{i,1}) = mean_data;
        ...
        idx_clusters_pvals = zeros(length(freqs)*size(times, 2), 1);
        idx_clusters_tmass = zeros(length(freqs)*size(times, 2), 1);

        signif_clusters = ([cluster.(fnames{i, 1}).p_values] <0.05);
        for k=1:sum(signif_clusters)
            idx_clusters(temp_cluster{1,k}) = 1;
            idx_clusters_pvals(temp_cluster{1,k}) = cluster.(fnames{i, 1}).p_values(1,k);
            idx_clusters_tmass(temp_cluster{1,k}) = cluster.(fnames{i, 1}).t_sums(k, 1);
        end
        idx_clusters2 = reshape(idx_clusters, [m n]);
        result_matrix_clusters_idx.(fnames{i, 1}) = idx_clusters2;
        result_matrix_clusters_pvals.(fnames{i, 1}) = reshape(idx_clusters_pvals, [m n]);
        result_matrix_clusters_tmass.(fnames{i, 1}) = reshape(idx_clusters_tmass, [m n]);
    end
    if  settings.prestimulusOnly == 1
        m = length(freqs);
        n = size(times, 2);
    ...
    results.result_matrix_clusters_idx = result_matrix_clusters_idx;
    results.result_matrix_clusters_pvals = result_matrix_clusters_pvals;
    results.result_matrix_clusters_tmass = result_matrix_clusters_tmass;
    results.result_mean_data = result_mean_data;
    ...
    mkdir([savepath 'betas_maps'])
    saveas(fig,[savepath '/betas_maps/' settings.prefix fnames{i,1} '.png']);
    saveas(fig,[savepath '/betas_maps/' settings.prefix fnames{i,1} '.fig']);
    close all

    clear idx_clusters* signif_clusters*
end

save([savepath 'results_exp_' num2str(settings.paradigm) '.mat']);
...
for i=1:length(fnames_topo)
    ...
    mkdir([savepath 'topo'])
    saveas(t,[savepath '/topo/' settings.prefix 'topo_betas' fnames_topo{i, 1} '.png']);
...
for i=1:length(fnames_topo)
    clear temp* mean_data mask_nonsignificant  mean_scalp signif_el mask_significant
    ...
    for n=1:length(settings.timesx)
        ...
        mkdir([savepath 'topo_separate_images'])
        saveas(t,[savepath '/topo_separate_images/' settings.prefix 'topo_betas' fnames_topo{i, 1} '.png']);
        saveas(t,[savepath '/topo_separate_images/' settings.prefix 'topo_betas' fnames_topo{i, 1} '.fig']);

        clear t mask_nonsignificant
        close all
        clear t
    end
end

data_topo.settings = settings;
data_topo.chanlocs = chanlocs;
data_topo.chanhood = chan_hood;
save([savepath 'topoplot_results_exp_' num2str(settings.paradigm) '.mat']);

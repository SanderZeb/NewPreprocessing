...
settings.prefix = '1_NEWnew_'; % additional prefix for naming plots
...
if settings.inverted== 1 & settings.intercept== 1
    ...
    pathBETAS = [root '\tfdata\betas_phase_odwrotne_intercept\']
...
elseif settings.inverted== 1 & settings.intercept== 0
    ...
    pathBETAS = [root '\tfdata\betas_phase_odwrotne\']
...
elseif settings.inverted == 0 & settings.intercept== 1
    ...
    pathBETAS = [root '\tfdata\betas_phase_intercept\']
...
elseif settings.inverted== 0 & settings.intercept== 0
    ...
    pathBETAS = [root '\tfdata\betas_phase\']
...
for s=1:length(listBetas)
    ...
    if  ~strcmp(file, 'betas.mat')
        ...
        C = regexp(file,'s_\w*chann','Match');

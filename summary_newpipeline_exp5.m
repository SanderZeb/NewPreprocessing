...
load('D:\Drive\5 - Scenes\participants version info.mat')
...
for i = 1:(participants_chans)
   ...
   list_exp_info(i) = str2num(IMPORTANT(i).participant(1:5));
...
for i=1:(participants_chans)
        ...
        data(i).exp_order = IMPORTANT(i).task_order
        temp_trials_part1 = [temp_trials.to_reject] <= 300;
        temp_trials_part2 = [temp_trials.to_reject] > 300;

        if data(i).exp_order == 1
            data(i).removed_trials_object = sum([temp_trials.to_reject] > 300);
            data(i).removed_trials_background = sum([temp_trials.to_reject] <= 300);
        elseif data(i).exp_order == 2
            data(i).removed_trials_object = sum([temp_trials.to_reject] <= 300);
            data(i).removed_trials_background = sum([temp_trials.to_reject] > 300);
        end

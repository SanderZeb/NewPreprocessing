...
paradigm = 1; % 1 - threshold; 2 - cue; 3 - mask; 4 - faces; 5 - scenes; 6 - Kinga
...
opts2 = delimitedTextImportOptions("NumVariables", 39, "Encoding", "UTF-8");
...
opts2.VariableTypes = ["double", "double", "double", "double", "double", "double", "double", "double", "double", "string", "double", "double", "double", "double", "double", "double", "double", "categorical", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "double", "string", "double", "string"];
...
% Specify variable properties
opts2 = setvaropts(opts2, ["image_name", "VarName39"], "WhitespaceRule", "preserve");
opts2 = setvaropts(opts2, ["image_name", "name", "VarName39"], "EmptyFieldRule", "auto");
...
for i=1:length(listEEGraw)
    ...
    if i ~= 29 & i ~= 46 &  i ~= 89 & i ~= 90 & i ~= 120 & i ~= 129
        % missing:
...
for s=[1:length(EEG_data_raw)]
    %if ~any(s==[7 8 29 30 45 46  58 89 90 101 120 129])
    if ~any(s==[7 8 29 30 45 46 58 89 90 101 120 129])
        % participants [7 8] 29 30 46 47 89 90 120 129 <-- bad recordings
        % participant 45 <-- EEG file is interrupted on 330 trial, behawior
        % contains 400 trials
        % participants 58 <-- EEG file is fine, behawior file is broken
        % participants 101 <-- EEG file interrupted, behav file interrupted
...
save('D:\Drive\Behawior\threshold_behawior_eeg.mat', "events");
...
for s=1:length(listEEGData)
    if ~any(s==[7 27 42 43 85 114 122])
        clear file idx
        ...
        for i = 1:length(curr_events2)
            if isempty(curr_events2(i).detection)
                idx(i) = 1;
            else
                idx(i) = 0;
            end
...
save('D:\Drive\1 - Threshold\events_new.mat', "events_new");

...
elseif settings.firstHalf ==1
    mkdir(pathTFData, 'fromSecondHalf');
    pathTFData2 = [pathTFData '\fromSecondHalf\'];
    if settings.inverted== 1 & settings.intercept== 1
        mkdir(pathTFData2, 'betas_odwrotne_intercept');
        pathBETAS = [root '\tfdatafromSecondHalf\betas_odwrotne_intercept\']
    elseif settings.inverted== 1 & settings.intercept== 0
        mkdir(pathTFData2, 'betas_odwrotne');
        pathBETAS = [root '\tfdata\fromSecondHalf\betas_odwrotne\']
    elseif settings.inverted == 0 & settings.intercept== 1
        mkdir(pathTFData2, 'betas_intercept');
        pathBETAS = [root '\tfdata\fromSecondHalf\betas_intercept\']
    elseif settings.inverted== 0 & settings.intercept== 0
        mkdir(pathTFData2, 'betas');
        pathBETAS = [root '\tfdata\fromSecondHalf\betas\']
    end
end
...
clear temp data* y* x* idx* beta* empty* participant_event participantID channel id_to_drop to_include idx_first_half idx_second_half
close all
for s=1:length(listTFData)
    ...
    if participantID~=34
        ...
        elseif settings.firstHalf == 1
            ...
            data = abs(temp.tfdata);
            data = data(:,:, logical(idx_first_half)); 
            participant_event = participant_event(:, logical(idx_first_half)); 
%         elseif settings.firstHalf == 1
%             task_type = participant_event(1).task_type;
% 
%             for i=1:length(participant_event)

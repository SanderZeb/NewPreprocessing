...
events = readtable("D:\Drive\1 - Threshold\exp1_alpha_behawior_final - Arkusz1.csv")
uniqueParticipants = unique(events.ID);
for i=1:length(uniqueParticipants)
    idx(i) = isnan(uniqueParticipants(i));
end
uniqueParticipants(idx) = [];
for i = 1:length(uniqueParticipants)
    events_new{i, 1} = events([events.ID]==uniqueParticipants(i), :);
    events_new{i, 2} = uniqueParticipants(i);
    for n = 1:length(listEEGData)
        if str2num(listEEGData(n).name(1:5)) == uniqueParticipants(i)
            events_new{i, 3} = n;
        end
    end
end
...
for s=1:length(listTFData)
    if contains(listTFData(s).name, 'tfdata_chan_') == 1
        ...
        if size(find([events_new{:, 3}] == listTFData(s).participant),2) ~= 0
            idx(s) = 0;
        else
            idx(s) = 1;
        end
...
for s=1:length(listTFData)
    ...
    participant_event = events_new{find([events_new{:,3}]==participantID), 1};

...
pathBETAS = [pathTFData '\betas_fooof\17\']
...
%%
listBetas=dir([ 'D:\Drive\1 - Threshold\tfdata\fooof2\betas_fooof\17\aperiodic\*.mat'  ]);
...
for i = 1:length(listBetas)
    ...
    elseif size(betas, 3) == 4
        all_betas.aperiodic_intercept(:,:,i) = betas(:,:,1);
...
%%
listBetas=dir([ 'D:\Drive\1 - Threshold\tfdata\fooof2\betas_fooof\17\periodic\*.mat'  ]);
...
for i = 1:length(listBetas)
    ...
    if size(betas, 3) == 3
        ...
        clear betas
    elseif size(betas, 3) == 4
        all_betas.periodic_intercept(:,:,i) = betas(:,:,1);

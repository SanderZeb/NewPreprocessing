...
pathBETAS = [pathTFData '\betas_fooof\2\']
...
for i = 1:length(listBetas)
    ...
    if contains(listBetas(i).name, '_backgrounds')
        if size(betas_backgrounds, 3) == 3
            all_betas.backgrounds_aperiodic_acc(:,:,i) = betas_backgrounds(:,:,1);
            all_betas.backgrounds_aperiodic_pas(:,:,i) = betas_backgrounds(:,:,2);
            all_betas.backgrounds_aperiodic_interaction(:,:,i) = betas_backgrounds(:,:,3);
            clear betas_backgrounds
        elseif size(betas_backgrounds, 3) == 4
            all_betas.backgrounds_aperiodic_acc(:,:,i) = betas_backgrounds(:,:,2);
            all_betas.backgrounds_aperiodic_pas(:,:,i) = betas_backgrounds(:,:,3);
            all_betas.backgrounds_aperiodic_interaction(:,:,i) = betas_backgrounds(:,:,4);
            clear betas_backgrounds
        end
    elseif contains(listBetas(i).name, '_objects')
        if size(betas_objects, 3) == 3
            all_betas.objects_aperiodic_acc(:,:,i) = betas_objects(:,:,1);
            all_betas.objects_aperiodic_pas(:,:,i) = betas_objects(:,:,2);
            all_betas.objects_aperiodic_interaction(:,:,i) = betas_objects(:,:,3);
            clear betas_objects
        elseif size(betas_objects, 3) == 4
            all_betas.objects_aperiodic_acc(:,:,i) = betas_objects(:,:,2);
            all_betas.objects_aperiodic_pas(:,:,i) = betas_objects(:,:,3);
            all_betas.objects_aperiodic_interaction(:,:,i) = betas_objects(:,:,4);
            clear betas_objects
        end
    end
end
...
for i = 1:length(listBetas)
    ...
    if contains(listBetas(i).name, '_backgrounds')
        if size(betas_backgrounds, 3) == 3
            all_betas.backgrounds_periodic_acc(:,:,i) = betas_backgrounds(:,:,1);
            all_betas.backgrounds_periodic_pas(:,:,i) = betas_backgrounds(:,:,2);
            all_betas.backgrounds_periodic_interaction(:,:,i) = betas_backgrounds(:,:,3);
            clear betas_backgrounds
        elseif size(betas_backgrounds, 3) == 4
            all_betas.backgrounds_periodic_acc(:,:,i) = betas_backgrounds(:,:,2);
            all_betas.backgrounds_periodic_pas(:,:,i) = betas_backgrounds(:,:,3);
            all_betas.backgrounds_periodic_interaction(:,:,i) = betas_backgrounds(:,:,4);
            clear betas_backgrounds
        end
    elseif contains(listBetas(i).name, '_objects')
        if size(betas_objects, 3) == 3
            all_betas.objects_periodic_acc(:,:,i) = betas_objects(:,:,1);
            all_betas.objects_periodic_pas(:,:,i) = betas_objects(:,:,2);
            all_betas.objects_periodic_interaction(:,:,i) = betas_objects(:,:,3);
            clear betas_objects
        elseif size(betas_objects, 3) == 4
            all_betas.objects_periodic_acc(:,:,i) = betas_objects(:,:,2);
            all_betas.objects_periodic_pas(:,:,i) = betas_objects(:,:,3);
            all_betas.objects_periodic_interaction(:,:,i) = betas_objects(:,:,4);
            clear betas_objects
        end
    end
end

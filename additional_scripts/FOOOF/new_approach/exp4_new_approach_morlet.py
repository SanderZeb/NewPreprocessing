...
matlab code:
for i = 1:length(events_new)
    if ~isempty(events_new{1,i})
        events_new2{i} = table2struct(events_new{1, i});
    else
        events_new2{i} = 0;
    end
end
save('D:\Drive\4 - Faces\events_for_python.mat', 'events_new2')
...
events = scipy.io.loadmat(r'D:\Drive\4 - Faces\events_new3.mat')
freqs_dict = loadmat(r'D:\freqs_morlet.mat')  # prestimulus part
events = events['events_new3']
freqs = np.squeeze(freqs_dict['freqs']).astype('float')

# csv_file =  r'C:\Users\user\Downloads\exp_4_alpha_behawior_final - Arkusz1.csv'
# with open(csv_file, 'r') as file:
#     reader = csv.reader(file)
#     data = [row for row in reader]
    

# path = r'D:\Drive\4 - Faces\Preprocessed_new_pipeline'
# file_list = [f for f in os.listdir(path) if f.endswith('.set')]


# for f in file_list:
#     name = f.split('_4.set')[0]
#     idx = int(name.split('_')[-1])
    
#     matching_data = [d for d in data if d[0] == name]
...
participants_to_drop = [60, 61, 62, 81];
# FOOOF settings
peaks = 5
lower_freqs = 6
upper_freqs = 20
...
for participant in range(events.shape[1]):
    if participant not in participants_to_drop:
        ...
        mat_file = scipy.io.loadmat(rf"D:\Drive\4 - Faces\pwelch_fromMorlet2\psd_{participant+1}.mat")
        ...
        incorrect_trials = np.where(events[0, participant]['accuracy'] ==  0)
...
scipy.io.savemat(r'D:\results_exp4_morlet5.mat', 

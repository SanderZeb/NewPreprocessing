...
for participant in range(1,129):
    ...
    for channel in selected_channels:
        ...
        with h5py.File(filename, 'r') as f:
            data = np.array(f['tfdata'])
            
        # Compute PSD
        psd = np.abs(data['real'] + 1j * data['imag'])**2
        ...
        aperiodic_results = np.zeros((35,200,data.shape[0]))
        periodic_results = np.zeros((35,200,data.shape[0]))
        for t in range(200):
        ...
        # Save results
        savemat(fr'D:\Drive\1 - Threshold\tfdata\fooof\fooof_chan_{channel}_participant_{participant}_aperiodic.mat', {'data': aperiodic_results})
        savemat(fr'D:\Drive\1 - Threshold\tfdata\fooof\fooof_chan_{channel}_participant_{participant}_periodic.mat', {'data': periodic_results})

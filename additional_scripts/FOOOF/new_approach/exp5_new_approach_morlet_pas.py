...
events = scipy.io.loadmat(r'D:\Drive\5 - Scenes\events.mat')
...
for participant in range(events.shape[1]):
    if participant not in participants_to_drop:
        ...
        mat_file = scipy.io.loadmat(rf"D:\Drive\5 - Scenes\pwelch_fromMorlet2\psd_{participant+1}.mat")
        ...
        correct_trials = np.where(
            (events[0, participant]['pas'] ==  [2 or 3 or 4]) & 
            (events[0, participant]['task_type'] == 'object'))
        incorrect_trials = np.where(
            (events[0, participant]['pas'] == 1) & 
            (events[0, participant]['task_type'] == 'object'))
...
scipy.io.savemat(r'D:\results_exp5_objects_morlet_pas.mat',
...
for participant in range(events.shape[1]):
    if participant not in participants_to_drop:
        ...
        correct_trials = np.where(
            (events[0, participant]['pas'] ==  [2 or 3 or 4]) & 
            (events[0, participant]['task_type'] == 'background'))
        incorrect_trials = np.where(
            (events[0, participant]['pas'] == 1) & 
            (events[0, participant]['task_type'] == 'background'))
...
incorrect = np.array(incorrect, dtype=object)
# save results back to .m file
scipy.io.savemat(r'D:\results_exp5_backgrounds_morlet_pas.mat',
                 {'results_correct': correct,
                  'results_incorrect': incorrect})
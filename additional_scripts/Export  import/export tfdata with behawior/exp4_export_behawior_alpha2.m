...
for s=1:length(new_list)
    ...
    if ~any(participantID==[82 61 62 63 ])
        ...
        if s+1 > length(new_list) || participantID ~= new_list(s+1).participant
            ...
            %	127 = UL45			157 = UR45
            %	126	= UL90			156 = UR90
            participant_event_clean = struct2table(participant_event_clean);
...
elseif settings.paradigm == 4
    writetable((all_data), ['D:\export\exp4_faces_behawior_alpha_new.csv'])
end

...
for s=1:length(new_list)
    ...
    %if participantID ~= 41 & participantID~= 66 & participantID~= 103 & participantID~= 142 & participantID~= 145 & participantID~= 162
        ...
        file = listEEGData(participantID).name;
        EEG = pop_loadset('filename',file,'filepath',listEEGData(1).folder);
        eegEvent = struct2table(EEG.event)
        eegEvent.Properties.VariableNames(1) = "typeEEG";
        eegEvent.Properties.VariableNames(2) = "latencyEEG";
        eegEvent.Properties.VariableNames(3) = "ureventEEG";
        eegEvent.Properties.VariableNames(4) = "identificationEEG";
        eegEvent.Properties.VariableNames(5) = "pasEEG";
        eegEvent.Properties.VariableNames(6) = "identification2EEG";
        eegEvent.Properties.VariableNames(7) = "stimulusEEG";
        eegEvent.Properties.VariableNames(8) = "epochEEG";
        ...
        try
        current = removevars(current, ["frameRate","VarName41","response","finalOpacity","postOri","stimulation_dur","post_fixation","stepSize","intensity", ...
            "image_name","global_start_time","global_end_time","pressKey","pressRT","releaseRT","image_started","image_ended","name", "y","maskFile","maskIndicator","mask"]);
        catch
        current = removevars(current, ["frameRate","response","finalOpacity","postOri","stimulation_dur","post_fixation","stepSize","intensity", ...
            "image_name","global_start_time","global_end_time","pressKey","pressRT","releaseRT","image_started","image_ended","name", "y","maskFile","maskIndicator","mask"]);
...
% %%% helper - find variables that don't match
% for i = 1:size(all_data, 2)
% idx(i) = strcmp(class(all_data{:, i}), class(participant_event_clean{:, i}))
% end

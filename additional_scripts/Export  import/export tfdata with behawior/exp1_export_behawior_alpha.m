...
for s=1:length(new_list)
    ...
    if ~any(participantID==[5 6 7 18 27 42 43 85 114 122 123])
        ...
        current = removevars(current, ["VarName39","response","finalOpacity","stepSize","intensity","image_name","global_start_time","global_end_time","pressKey","pressRT","releaseRT","image_started","image_ended","name"]);
...
for i = 1:size(events,2)
    if isempty(events{1,i})
        idx_missing_files(i) = 1;
    else
        idx_missing_files(i) = 0;
    end
end


length(unique([all_data.ID]))
sum(idx_missing_files)
length(unique([new_list.participant]))
missing_files = listEEGData(logical(idx_missing_files))
missing_size_GB = [missing_files.bytes]/1000000
